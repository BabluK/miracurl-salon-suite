  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden rounded-2xl border border-[#d4af37]/40 bg-[#15151b] text-slate-100 p-5" data-testid="miracurl-updates-card">
        ...
        {d.status === "ended" && c.payment_link && <div className="relative mt-3 rounded-xl border border-emerald-400/40 bg-emerald-500/10 px-4 py-2.5 text-xs text-emerald-200" data-testid="miracurl-updates-payment-note"><b>Settlement:</b> {c.payment_note || "Please complete the campaign settlement using the payment link."}</div>}
        {c.updates?.length > 0 && (
