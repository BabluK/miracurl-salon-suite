...
export function HiringEarningsReview() {
  ...
  if (!fees) return null;
  const rows = fees.items.filter(f => filter === "all" || f.status === filter);

  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm" data-testid="placement-fees-card">
      <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
        <div>
          <h3 className="text-sm font-semibold text-slate-800">🤝 Staff Hiring — Earnings Review <span className="font-normal text-slate-400">· ₹{Number(fees.fee_per_hire).toLocaleString("en-IN")} per hire</span></h3>
          <p className="text-[11px] text-slate-400 mt-0.5">Every placement fee from the hiring marketplace — review, chase dues and mark paid.</p>
        </div>
      ...
      <div className="flex gap-1.5 mb-2">
        {[["all", "All"], ["due", "Due"], ["paid", "Paid"]].map(([k, l]) => (
          <button key={k} onClick={() => setFilter(k)} data-testid={`fees-filter-${k}`}
            className={`px-3 py-1 rounded-full text-[11px] font-semibold ${filter === k ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>{l}</button>
        ))}
      </div>
      {rows.length === 0 && <p className="text-xs text-slate-400 italic py-3">No placement fees {filter !== "all" ? `with status "${filter}"` : "yet — fees appear here automatically when a marketplace candidate is hired"}.</p>}
          ...
          <div key={f.id} className="flex items-center justify-between gap-3 py-2 text-sm flex-wrap" data-testid={`placement-fee-${f.id}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              {f.status === "due" ? (
                ...
                <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 font-medium">paid{f.paid_at ? ` · ${new Date(f.paid_at).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}` : ""}</span>
              )}
            </div>
          </div>
        ))}
