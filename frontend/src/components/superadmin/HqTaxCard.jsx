...
// Declared at module level: defining it inside the card would remount the <input> on every keystroke (focus + text lost).
const Field = ({ k, label, ph, hint, value, onChange }) => (
  <label className="block"><span className="text-xs font-semibold text-slate-600">{label}</span>
    <input value={value ?? ""} onChange={onChange} placeholder={ph} className="mt-1 w-full rounded-lg border border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#b8860b]/40" data-testid={`hq-tax-${k}`} />
...
export const HqTaxCard = () => {
  ...
  const f = (k) => ({ k, value: p[k], onChange: set(k) });
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5" data-testid="hq-tax-card">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <Field {...f("legal_name")} label="Legal name on invoices" ph="Miracurl Studio" />
        <Field {...f("gst_rate_pct")} label="GST rate %" ph="18" hint="Default 18% (SaaS)" />
        <Field {...f("gstin")} label="GSTIN" ph="29ABCDE1234F1Z5" hint={p.gstin ? "" : "Not received yet — GST is still charged; invoices say 'GSTIN pending'"} />
        <Field {...f("msme")} label="MSME / Udyam number" ph="UDYAM-KR-03-0012345" />
        <div className="sm:col-span-2"><Field {...f("address")} label="Registered address (invoice footer)" ph="Bengaluru, Karnataka" /></div>
      </div>
