        ...
        <div className="hidden lg:flex items-center gap-5 text-sm">
          ...
          <div className="relative" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
            <button onClick={() => setOpen((v) => !v)} data-testid="header-contact-btn"
              className={`flex items-center gap-1 transition-colors ${t.link}`}>
          ...
          <Link to="/login" className={`${t.link} font-medium transition-colors`} data-testid="header-signin-link">Sign In</Link>
          <Link to="/signup-salon" data-testid="header-signup-link"
            className="px-4 py-2 rounded-full bg-gradient-to-b from-[#F0D9A5] to-[#C89B52] text-[#1c160c] text-sm font-bold hover:brightness-110 hover:-translate-y-0.5 shadow-[0_8px_24px_-6px_rgba(200,155,82,0.5)] transition-transform">
            Sign Up
          </Link>
        </div>
