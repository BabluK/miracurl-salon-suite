    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-ink-secondary text-sm mt-1">Curate what Miracurl offers your guests.</p>
        </div>
        <button data-testid="add-service-btn" onClick={startNew} className="btn-gold flex items-center gap-2">
      ...
      {Object.keys(byCategory).map(cat => (
        <div key={cat}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs text-ink-muted">{byCategory[cat].length} services</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {byCategory[cat].map(s => (
              <div key={s.id} data-testid={`service-card-${s.id}`} className="card-luxe p-0 overflow-hidden group hover:border-gold/30 transition-all">
                <div className="h-32 relative">
                  ...
                  {s.trending && (
                    <span className="absolute top-2 left-2 bg-gold text-bg-base text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded flex items-center gap-1">
                ...
                <div className="p-4">
                  ...
                  {s.description && <p className="text-xs text-ink-secondary mt-1 line-clamp-2">{s.description}</p>}
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-3 text-sm">
                      <span className="flex items-center gap-1 text-gold font-semibold"><IndianRupee className="w-3 h-3" />{s.price}</span>
                      <span className="flex items-center gap-1 text-ink-secondary text-xs"><Clock className="w-3 h-3" />{s.duration_min}m</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <button data-testid={`edit-service-${s.id}`} onClick={() => startEdit(s)} className="p-1.5 hover:bg-white/5 rounded text-ink-secondary hover:text-gold"><Edit3 className="w-3.5 h-3.5" /></button>
                      <button data-testid={`delete-service-${s.id}`} onClick={() => remove(s.id)} className="p-1.5 hover:bg-red-500/10 rounded text-ink-secondary hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="card-luxe w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={save} className="space-y-4">
              <div><label className="label-luxe block mb-1">Name *</label><input data-testid="service-name-input" required className="input-luxe" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
              <div className="grid grid-cols-3 gap-3">
                <div><label className="label-luxe block mb-1">Category</label>
                ...
                <div><label className="label-luxe block mb-1">Duration (min)</label><input type="number" required className="input-luxe" value={form.duration_min} onChange={e => setForm({ ...form, duration_min: e.target.value })} /></div>
              </div>
              ...
              <div><label className="label-luxe block mb-1">Description</label><textarea rows="2" className="input-luxe" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button data-testid="save-service-btn" type="submit" className="btn-gold flex-1">{editing ? "Update" : "Create"}</button>
              </div>
            </form>
          </div>
        </div>
