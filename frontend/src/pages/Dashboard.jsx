        ...
        <Stat icon={TrendingUp} label="This Month" color="amber" testid="kpi-revenue-month" now={monthMasked ? null : (data.month_revenue ?? peekValue)} prev={monthMasked || data.month_revenue == null ? null : data.compare?.last_month_revenue} vs="vs last month"
          ...
          action={isAdmin && (
            <button onClick={toggleRevenueLock} data-testid="month-revenue-lock-btn"
              title={data.month_revenue_hidden_for_staff ? "Month revenue, Revenue Trend & Business by Stylist are hidden from managers/staff — tap to show them (owner PIN)" : "Visible to managers/staff — tap to hide month revenue, Revenue Trend & Business by Stylist (owner PIN)"}
  ...
  return (
    <div data-testid="perf-locked" className="relative rounded-2xl border border-dashed border-[#e3d5bd] bg-white/60 px-5 py-8 sm:py-10 text-center overflow-hidden">
      <div className="pointer-events-none absolute inset-0 opacity-60" aria-hidden="true">
        {[0, 1, 2].map(i => (
          <div key={i} className="mx-5 sm:mx-10 my-3 h-12 rounded-xl bg-[#f3e9d8]/70 blur-[2px]" style={{ width: `${88 - i * 14}%` }} />
        ))}
      </div>
      <div className="relative flex flex-col items-center gap-3 animate-fade-up">
        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#d4af37] to-[#b8893a] text-white flex items-center justify-center shadow-[0_8px_24px_rgba(201,162,74,.35)] ring-4 ring-white">
    ...
    <div className="relative overflow-hidden rounded-3xl border border-[#eadfcb] p-5 sm:p-6 shadow-sm bg-[radial-gradient(120%_100%_at_0%_0%,#fff8ec_0%,#f7efe2_55%,#f3e9d8_100%)]" data-testid="staff-performance-card">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4 mb-5">
        ...
        <div className="flex items-center gap-1 p-1 rounded-full bg-white/80 border border-[#eadfcb] shadow-sm flex-wrap" data-testid="perf-tabs">
          ...
          {PERF_TABS.map(t => (
            <button key={t.k} data-testid={`perf-tab-${t.k}`} onClick={() => setTab(t.k)} disabled={showLock}
              className={`px-3.5 py-1.5 rounded-full text-xs font-semibold transition-[background-color,color,transform] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
  ...
  return (
    <div className={`${GOLD_CARD} lg:col-span-2`} data-testid="revenue-trend-card">
      <div className="flex items-start justify-between flex-wrap gap-3 mb-3">
        ...
        {first && !showLock && <div className="inline-flex items-center gap-2 px-3.5 py-2 rounded-full bg-white/80 border border-[#eadfcb] text-xs font-medium text-slate-700 shadow-sm" data-testid="revenue-trend-range"><Calendar className="w-3.5 h-3.5 text-[#b08d3f]" /> {fmtDay(first)} – {fmtDay(last)} {last?.slice(0, 4)}</div>}
      </div>
      {showLock ? (
        <div data-testid="revenue-trend-locked" className="relative h-64 rounded-2xl border border-dashed border-[#e3d5bd] bg-white/60 overflow-hidden flex items-center justify-center">
          <svg className="pointer-events-none absolute inset-x-6 bottom-6 h-32 w-[calc(100%-3rem)] opacity-50 blur-[2px]" viewBox="0 0 600 120" preserveAspectRatio="none" aria-hidden="true">
            <path d="M0 100 C 80 60, 140 110, 200 70 S 320 20, 400 60 S 520 110, 600 40 L600 120 L0 120 Z" fill="#e6d3ad" />
            <path d="M0 100 C 80 60, 140 110, 200 70 S 320 20, 400 60 S 520 110, 600 40" fill="none" stroke="#c9a24a" strokeWidth="3" />
          </svg>
          <div className="relative flex flex-col items-center gap-3 px-5 text-center animate-fade-up">
      ...
      {!showLock && <div className="mt-4 pt-4 border-t border-[#e3d5bd] grid grid-cols-2 sm:grid-cols-4 gap-3" data-testid="revenue-trend-stats">
