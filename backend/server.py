...
from routes.wa_coexist import router as wa_coexist_router  # noqa: E402
from routes.hq_credit_wallet import router as hq_credit_wallet_router  # noqa: E402
...
from schedulers import (  # noqa: E402
    _colour_price_reminder_scheduler, _banner_schedule_scheduler, _monthly_report_scheduler, _weekly_report_scheduler, _birthday_scheduler,
    ...
    _loyalty_nudge_scheduler, _always_on_time_scheduler, _referral_nudge_scheduler, _earnings_anomaly_scheduler,
)
...
async def _boot_wa_campaigns():
    ...
    from sms_service import apply_hq_sms_template_ids
    if await apply_hq_sms_template_ids():
        logging.getLogger("sms").info("HQ MSG91 template IDs applied from hq_settings")
...
# SEC (audit #4): no wildcard platform regex — only the explicit CORS_ORIGINS list is trusted with credentials.
# The app is served same-origin in production, so its own host never needs a CORS entry.
_cors_regex = None
app.add_middleware(
    CORSMiddleware,
