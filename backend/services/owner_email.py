...
async def change_owner_email(t: dict, new_email: str, scope: str = "all", takeover: bool = False, by: str = "hq") -> dict:
    ...
    if existing and owner and existing["id"] == owner["id"]:
        ...
        result["mode"] = "linked_existing_owner"
        result["owner_salon_count"] = len(sets["tenant_ids"])
        if owner:
            result.update(await _detach_owner(owner, tid))
    elif existing:
        if not takeover:
        ...
        await _raw_db.users.update_one(
            ...
            {"$set": {"role": "admin", "tenant_id": tid, "tenant_ids": sorted(owned_tenant_ids(existing) | {tid}),
             ...
             "$unset": {"branch": ""}})
        result["mode"] = f"took_over_{existing.get('role') or 'user'}_login"
        if owner:
            result.update(await _detach_owner(owner, tid))
        ...
        result.update({"mode": "renamed_login", "branches_updated": r.modified_count})
    else:
        ...
        await _raw_db.users.insert_one({
            ...
            "role": "admin", "tenant_id": tid, "tenant_ids": [tid], "status": "active",
            "password_hash": hash_pw(temp_pw), "must_change_password": True, "created_at": now, "created_by": by})
        if owner:
            result.update(await _detach_owner(owner, tid))
        result.update({"mode": "created_login", "temp_password": temp_pw,
                       "email_status": await _email_new_credentials(t, new_email, temp_pw)})
