...
BUILD = "2026-09-21.340"
BUILD_TIME = "21 Sep 2026, 6:10 PM IST"
...
BUILD_LOG = [
    ...
    {"build": "2026-09-21.339", "note": "seeds.retire_default_super_admin_v2: on boot, if any other super_admin exists the dummy super@miracurl.com is set role=retired_seed (one-shot via app_migrations) — prod HQ login stays super@miracurl-suite.com. HQ import undo: POST /super-admin/tenants/{tid}/customers/import-clean {date, action: preview|delete|move, target_tid} — guests created that local day with 0 visits/₹0; move skips phones already in target (dups deleted); audit_log hq_import_clean. UI: ImportCustomersModal 'Wrong salon? Undo / move an import…' panel. WA campaigns: capped batches store capped_until (usage resets_at); history header shows '⏸ daily limit reached — resumes …' instead of 'batch N sending now'; per-row hint; composer warns when auto-batch total > tenant wa_daily_cap."},
    {"build": "2026-09-21.338", "note": "Gift-card breakage: expiry job (deliver_scheduled_gift_cards) now stamps expired_at + breakage_amount=$balance (pipeline update, backfills old expired cards); GET /gift-cards stats.breakage/expired_count, /gift-cards/analytics months[].breakage_amount; /reports/daily + /reports/sales return gift_card_breakage + gift_cards_expired (_gift_card_breakage by expired_at range). Validity min 7 days (GiftSettingsIn ge=7; settings select adds 7 days); POS GiftCardSellModal 'Valid for' chips → gift_meta.validity_days (7–365, else salon default). Missed-bill nudge: GET /appointments/unbilled-recent (completed, last BACKDATE_MAX_DAYS local days, no non-void invoice, not dismissed) + POST /appointments/{aid}/dismiss-unbilled; dashboard/MissedBillNudge.jsx (all roles) → /pos?appointment=<id>&bill_date=<day> pre-sets the back-date."},
...
RELEASES = [
    {
        ...
        "changes": [
            "🎁 Gift cards → breakage revenue: when a card expires with balance left, that amount is now recorded and shown as 'Expired / breakage' in Settings → Gift Cards, in the 6-month chart, in Yesterday's Report and in Reports → Total Sales. Validity can now be as short as 7 days, and staff can pick a different 'Valid for' per card while selling at the POS",
            "🧾 Missed bills nudge on the Dashboard (owner, manager & staff): completed appointments from the last 2 days that still have no bill. 'Bill now' opens the POS pre-filled with the booking AND the bill date already set to that day; 'Dismiss' hides ones where the guest didn't pay",
    ...
    },
    {
        ...
        "changes": [
            "👥 Adding a manager with an email that already has a login in your group no longer fails with 'Email already registered' — the login is adopted as the manager (password kept), linked to every business, and a Staff profile with the salary and commission you typed is created so they appear in Staff & bookings",
            "📍 Managers & staff are asked for their location EVERY time they open the app — a branch can only be opened from inside it (100 m). Only the owner can set or move branch GPS pins and change login details",
            "🔑 Owners choose manager passwords: type one while adding a manager, or tap 'Set password' on any manager — works immediately, no temp password or forced change. Managers now always appear in Staff of every business they cover",
            "🛡️ Fixed: a deleted default owner login could quietly come back after a restart. It now stays deleted",
        ],
    },
    {
