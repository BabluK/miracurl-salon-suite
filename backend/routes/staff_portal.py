...
@router.get("/staff/me/attendance.pdf")
async def staff_my_attendance_pdf(month: Optional[str] = None, s=Depends(_current_staff), t=Depends(current_tenant)):
    """Full-month attendance sheet as PDF (the page shows 10 rows at a time; this has every day)."""
    from fastapi.responses import Response
    from reportlab.lib import colors
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.styles import getSampleStyleSheet
    from reportlab.platypus import Paragraph, SimpleDocTemplate, Spacer, Table, TableStyle
    data = await staff_my_attendance(month=month, s=s)
    recs = sorted(data["records"], key=lambda r: r.get("date") or "")

    def hhmm(iso):
        if not iso:
            return "—"
        try:
            return datetime.fromisoformat(iso.replace("Z", "+00:00")).astimezone(IST_TZ).strftime("%I:%M %p")
        except ValueError:
            return "—"
    ...
    def status(r):
        ...
        if r.get("half_day"):
            return "Half day"
        if r.get("check_in_at") and not r.get("check_out_at"):
            return "Not checked out"
        return "Present" if r.get("check_in_at") else "—"

    rows = [["Date", "Day", "Check in", "Check out", "Hours", "Status"]]
    for r in recs:
        d = datetime.strptime(r["date"], "%Y-%m-%d")
        rows.append([d.strftime("%d %b %Y"), d.strftime("%a"), hhmm(r.get("check_in_at")), hhmm(r.get("check_out_at")),
                     f"{float(r.get('hours_worked') or 0):.2f}" if r.get("hours_worked") else "—", status(r)])
    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=A4, leftMargin=36, rightMargin=36, topMargin=40, bottomMargin=36)
    st = getSampleStyleSheet()
    title = datetime.strptime(data["month"] + "-01", "%Y-%m-%d").strftime("%B %Y")
    story = [Paragraph(f"<b>{t.get('name', 'Salon')}</b> — Attendance sheet", st["Title"]),
             Paragraph(f"{s.get('name', '')} · {title} · Present days: {data['total_days']} · Hours: {data['total_hours']}", st["Normal"]), Spacer(1, 12)]
    tbl = Table(rows, repeatRows=1, colWidths=[85, 45, 80, 80, 55, 110])
    tbl.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#1a1408")), ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#e8c56a")),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("FONTSIZE", (0, 0), (-1, -1), 9),
        ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.HexColor("#fbf6e9")]),
        ("GRID", (0, 0), (-1, -1), 0.25, colors.HexColor("#e6d3a3")), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
    ]))
    story.append(tbl)
    story.append(Spacer(1, 14))
    story.append(Paragraph("Generated by Miracurl Salon Suite", st["Italic"]))
    doc.build(story)
    return Response(buf.getvalue(), media_type="application/pdf",
                    headers={"Content-Disposition": f'inline; filename="attendance-{data["month"]}.pdf"'})
...
async def _compute_salary_for_month(staff: dict, year: int, month: int, tenant: dict) -> dict:
    ...
    # Month boundaries in salon time (IST) → UTC ISO, so a bill at 11 pm IST on the 31st stays in ITS month
    start = datetime(year, month, 1, tzinfo=IST_TZ).astimezone(timezone.utc).isoformat()
    end = datetime(year, month, end_day, 23, 59, 59, tzinfo=IST_TZ).astimezone(timezone.utc).isoformat()
