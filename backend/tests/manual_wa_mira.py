...
async def main():
    r1 = await wc.process_webhook_payload(payload(f"wamid.t{int(time.time())}a", "Hi"))
    r2 = await wc.process_webhook_payload(payload(f"wamid.t{int(time.time())}b", "price list"))
    print("counts", r1, r2)
