...
## Iter 149 (23 Aug 2026) — Google rating badge + dual-PWA install guidance
- Booking pages now show a Google-style rating badge under the hero (blue score circle + EXCELLENT/GREAT/GOOD + stars + "Rated by N customers on Google", links to google_review_url). Data: new _salon_rating() in public_site.py = avg of google_reviews_archive + in-app reviews; badge hidden below 3.5 avg to protect tenants. Verified on /book/miracurl-marathahalli (3.9 GOOD, 73).
- Dual PWA (Miracurl Partner scope "/" vs Miracurl Booking scope "/book/"): ids already distinct; Android Chrome suppresses install when page URL is inside an ALREADY-INSTALLED app's scope (per Google docs, overlapping scopes discouraged). WORKAROUND (told user): install Miracurl Booking FIRST from /book page, THEN Miracurl Partner from /login — both coexist. iOS unaffected. Real fix would need /partner route migration or booking subdomain — backlog P2.

## Iter 150 (23 Aug 2026) — WhatsApp gate + Review Booster + PWA naming
- ChatButton (signup page "Need help?" widget): "Chat on WhatsApp" now GATED — requires name+phone+valid email, saves lead via POST /public/demo-request (source whatsapp_gate → HQ inbox), THEN opens https://wa.me/message/LMGKRXVV2SHVB1 (business QR link). Done-state shows "Continue on WhatsApp". Verified: empty form blocked with red hint.
